...
const gtfs_routes = require('./routes.json');
...
_Routes.prototype.fetchBy_human_id = (route_id) => {
    ...
    if( route_id.length < 2 && parseInt(route_id) < 10 ) {
      route_id_string += '0' + route_id;
    } else {
        route_id_string += route_id;
    }
    const route = _.find(gtfs_routes, (route) => {
        return (route.route_short_name == route_id_string) && (route.service_id == service.getActiveServiceID());
    });
    console.dir(route);
    return route;
}
